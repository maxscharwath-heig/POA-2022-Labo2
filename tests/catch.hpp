...
namespace Catch {
    ...
    constexpr auto
    operator "" _sr(char const* rawChars, std::size_t size) noexcept -> StringRef {
...
namespace Catch {
    ...
    struct is_callable_tester {
        ...
        true_given<decltype(std::declval<Fun>()(std::declval<Args>()...))> static
        test(int);
...
namespace Catch {
    namespace Detail {
        ...
        public:
            static const bool value = decltype(test<std::ostream, const T&>(
                  0))::value;
        };
    ...
    struct is_range {
        static const bool value =
              !std::is_same<decltype(begin(
                    std::declval<T>())), not_this_one>::value &&
    ...
    class BinaryExpr : public ITransientExpression {
        ...
        void streamReconstructedExpression(std::ostream& os) const override {
            formatReconstructedExpression
                  (os, Catch::Detail::stringify(m_lhs), m_op,
    ...
    namespace Matchers {
        ...
        namespace Vector {
            ...
            struct ContainsElementMatcher : MatcherBase<std::vector<T, Alloc>> {
                ...
                ContainsElementMatcher(T const& comparator) : m_comparator(
                      comparator) {}
...
namespace Catch {
    ...
    namespace Generators {
        ...
        class FixedValuesGenerator final : public IGenerator<T> {
            ...
            size_t m_idx = 0;
        public:
            FixedValuesGenerator(std::initializer_list<T> values) : m_values(
                  values) {}
...
//       but I don't expect users to run into that in practice.
        ...
        public:
            ...
            IteratorGenerator(InputIterator first, InputSentinel last):m_elems(first,
                                                                               last) {
                if (m_elems.empty()) {
                    Catch::throw_exception(GeneratorException(
    ...
    namespace Detail {
        ...
        bool Approx::equalityComparisonImpl(const double other) const {
            ...
            return marginComparison(m_value, other, m_margin)
                   || marginComparison(m_value, other, m_epsilon * (m_scale +
                                                                    std::fabs(
                                                                          std::isinf(
                                                                                m_value)
                                                                          ? 0
...
namespace Catch {
    ...
    LazyExpression::LazyExpression(LazyExpression const& other) : m_isNegated(
          other.m_isNegated) {}
        ...
        namespace TextFlow {
            ...
            public:
                class iterator {
                    ...
                    auto
                    line() const -> std::string const& { return m_column.m_strings[m_stringIndex]; }
                    ...
                    auto addIndentAndSuffix(
                          std::string const& plain) const -> std::string {
...
namespace Catch {
    namespace clara {
        namespace detail {
            ...
            public:
                explicit TokenStream(Args const& args) : TokenStream(
                      args.m_args.begin(), args.m_args.end()) {}
            ...
            public:
                ...
                auto
                remainingTokens() const -> TokenStream { return m_remainingTokens; }
    ...
    inline auto convertInto( std::string const &source, CLARA_CONFIG_OPTIONAL_TYPE<T>& target ) -> ParserResult {
        ...
        return result;
            ...
            protected:
                ...
                explicit ParserRefImpl(std::shared_ptr<BoundRef> const& ref) : m_ref(
                      ref) {}
            ...
            class ExeName : public ComposableParserImpl<ExeName> {
                ...
                template<typename LambdaT>
                static auto makeRef(
                      LambdaT const& lambda) -> std::shared_ptr<BoundValueRefBase> {
            ...
            public:
                ...
                explicit Opt(bool& ref) : ParserRefImpl(
                      std::make_shared<BoundFlagRef>(ref)) {}

                template<typename LambdaT>
                Opt(LambdaT const& ref, std::string const& hint) : ParserRefImpl(ref,
                                                                                 hint) {}
                ...
                void writeToStream(std::ostream& os) const {
                    ...
                    for (auto const& cols: rows) {
                        auto row =
                              ...
                              TextFlow::Spacer(4) +
                              TextFlow::Column(cols.right).width(
                                    consoleWidth - 7 - optWidth);
                        os << row << std::endl;
                    }
                }
            ...
            template<typename DerivedT>
            template<typename T>
            auto ComposableParserImpl<DerivedT>::operator|(
                  T const& other) const -> Parser {
...
namespace Catch {
    ...
    std::string const&
    Config::getReporterName() const { return m_data.reporterName; }
    ...
    bool
    Config::includeSuccessfulResults() const { return m_data.showSuccessfulTests; }
    ...
    ShowDurations::OrNot
    Config::showDurations() const { return m_data.showDurations; }
    ...
    unsigned int
    Config::benchmarkResamples() const { return m_data.benchmarkResamples; }
...
namespace Catch {
    ...
    std::size_t listTests(Config const& config) {
        ...
        for (auto const& testCaseInfo: matchedTestCases) {
            ...
            if (config.verbosity() >= Verbosity::High) {
                Catch::cout() << Column(
                      Catch::Detail::stringify(testCaseInfo.lineInfo)).indent(4)
                              << std::endl;
    ...
    std::size_t listTestsNamesOnly(Config const& config) {
        ...
        std::vector<TestCase> matchedTestCases = filterTests(
...
#pragma clang diagnostic pop
            ...
            bool WithinRelMatcher::match(double const& matchee) const {
                const auto relMargin = m_epsilon *
                                       (std::max) (std::fabs(matchee), std::fabs(
                                             m_target));
...
//           case '>':
                ...
                case ',':
                    if (start != pos && openings.empty()) {
                        ...
                        m_messages.back().message = static_cast<std::string>(trimmed(
                              start, pos));
...
namespace Catch {
    ...
    namespace Generators {
        struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
            ...
            static GeneratorTracker& acquire(TrackerContext& ctx,
                ...
                if (TestCaseTracking::ITrackerPtr childTracker = currentTracker.findChild(
                      nameAndLocation)) {
                    assert(childTracker);
    ...
    RunContext::testGroupEnded(std::string const& testSpec, Totals const& totals,
        ...
        m_reporter->testGroupEnded(
              TestGroupStats(GroupInfo(testSpec, groupIndex, groupsCount), totals,
                             aborting()));
    }
...
namespace Catch {
    ...
    namespace {
        ...
        void applyFilenamesAsTags(Catch::IConfig const& config) {
            auto& tests = const_cast<std::vector<TestCase>&>(getAllTestCasesSorted(
                  config));
...
namespace Catch {
    TagAlias::TagAlias(std::string const& _tag, SourceLineInfo _lineInfo) : tag(
...
namespace Catch {
    ...
    void TagAliasRegistry::add(std::string const& alias, std::string const& tag,
        ...
        CATCH_ENFORCE(m_registry.insert(
              std::make_pair(alias, TagAlias(tag, lineInfo))).second,
...
// Implementation of CompactReporter formatting
        ...
        private:
            ...
            void printRemainingMessages(Colour::Code colour = dimColour()) {
                ...
                const auto N = static_cast<std::size_t>(std::distance(itMessage,
                                                                      itEnd));
...
namespace Catch {
    ...
    JunitReporter::writeGroup(TestGroupNode const& groupNode, double suiteTime) {
        ...
        xml.scopedElement("system-out").writeText(trim(stdOutForSuite),
                                                  XmlFormatting::Newline);
